...
- [ ] can't we make a ObSync plugin handle git? Like why do we need our app, that purpose was only to host a git in the android through JGit, if the plugin can handle that then there's no need for our app the we need to find a way to include GitHub onboarding and stuffs in plugin is it possible, let it not be possible lol 
